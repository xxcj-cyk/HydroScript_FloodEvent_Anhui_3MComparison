...
csv_path = r"./Data/Select/anhui20_705.csv"
# rs_list = [1111, 2222, 3333, 4444, 5555]
rs_list = [6666, 7777, 8888, 9999]

def load_basin_ids(csv_path):
    basin_data = pd.read_csv(csv_path)
    return basin_data["basin"].tolist()

def lstm_hydrodataset_args(basin_ids, seed):
    project_name = os.path.join("Anhui_LSTM", f"anhui20_705_b20_fl60_seed{seed}")
...
def run_lstm_exp(csv_path, seed):
    cfg = default_config_file()
    basin_ids = load_basin_ids(csv_path)
    args_ = lstm_hydrodataset_args(basin_ids, seed)
...
def main():
    with ProcessPoolExecutor(max_workers=5) as executor:
        futures = [executor.submit(run_lstm_exp, csv_path, seed) for seed in rs_list]
