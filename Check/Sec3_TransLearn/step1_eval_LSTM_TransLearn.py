...
configuration_name = "_b05_fl72_lr0005_seed1111_tl_weight"
basin_names = [
    "anhui_50406910_20",
    "anhui_50501200_34",
    "anhui_50701100_41",
...
def step1_extract_validation_metrics(current_project_name):
    ...
    output_dir = f"./Visualization/Sec3_TransLearn/{model_name}/{current_project_name}"
...
def lstm_hydrodataset_args(basin_ids, current_project_name):
    ...
    trained_weight_path = os.path.join(
    ...
    return cmd(
        ...
        loss_func="RMSEFlood",
        # 10. 评估配置
        model_loader={"load_way": "pth", "pth_path": trained_weight_path},
        fill_nan=["no"],
...
def step3_merge_training_validation_data(current_project_name):
    ...
    output_csv = f"./Visualization/Sec3_TransLearn/{model_name}/{current_project_name}/{model_name}_{project_name}_metrics.csv"
    valid_csv = f"./Visualization/Sec3_TransLearn/{model_name}/{current_project_name}/{model_name}_{current_project_name}_valid_metrics.csv"
    combined_csv = f"./Visualization/Sec3_TransLearn/{model_name}/{current_project_name}/{model_name}_{current_project_name}_metrics.csv"
...
def step4_standardize_data(current_project_name):
    ...
    combined_csv = f"./Visualization/Sec3_TransLearn/{model_name}/{current_project_name}/{model_name}_{current_project_name}_metrics.csv"
    final_csv = f"./Visualization/Sec3_TransLearn/{model_name}/{current_project_name}/{model_name}_{current_project_name}.csv"
...
def step5_merge_final_csvs():
    ...
    for project in base_project_name:
        final_csv = f"./Visualization/Sec3_TransLearn/{model_name}/{project}/{model_name}_{project}.csv"
    ...
    if all_dfs:
        ...
        merged.to_csv(
            f"./Visualization/Sec3_TransLearn/{model_name}/{model_name}_Merged{configuration_name}.csv",
            index=False,
        )
        print(
            f"所有项目结果已合并到: ./Visualization/Sec3_TransLearn/{model_name}/{model_name}_All_Merged.csv"
        )
    else:
        print("没有找到可合并的csv文件。")
...
def main():
    ...
    try:
        ...
        for i, current_project in enumerate(base_project_name):
            ...
            print(
                f"结果文件: ./Visualization/Sec3_TransLearn/{model_name}/{current_project}/{model_name}_{current_project}.csv"
            )
