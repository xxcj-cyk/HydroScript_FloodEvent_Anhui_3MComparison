# -*- coding: utf-8 -*-
# @Author: Yikai CHAI
# @Date:   2025-06-20 23:20:54
# @Last Modified by:   Yikai CHAI
# @Last Modified time: 2025-09-05 13:57:49
import os
import pandas as pd
...
def get_project_name(csv_path, target_basin_code):
    ...
    return os.path.join("Anhui_LSTM_TL", f"{filename}_b02_fl72_lr0005_rs1111_tl_{source_filename}")
...
def get_model_and_stat_paths(target_basin_code):
    ...
    # 构建模型和统计文件路径
    base_dir = "Result/Sec2_ModelPerf/Anhui_LSTM"
    model_dir = f"{source_filename}_b05_fl72_lr0005_rs1111"
...
def lstm_hydrodataset_args(basin_ids, project_name, model_path, stat_file_path):
    train_period = ["2024-07-01 00:00:00", "2024-07-31 23:00:00"]
    valid_period = ["2024-08-01 00:00:00", "2024-08-31 23:00:00"]
    test_period = ["2024-07-01 00:00:00", "2024-08-31 23:00:00"]
    return cmd(
        # 1. 项目和基础配置
        sub=project_name,
        ctx=[2],
...
def run_single_lstm_exp(csv_path):
    ...
    args_ = lstm_hydrodataset_args(basin_ids, project_name, model_path, stat_file_path)
...
def run_sequential_lstm_exp(csv_paths):
    ...
    for csv_path in csv_paths:
        try:
            result = run_single_lstm_exp(csv_path)
            if result:
                success_count += 1
            else:
                failed_count += 1
