...
# @Last Modified time: 2025-09-02 18:56:43
import os
import json
import glob
import xarray as xr
...
base_project_prefix = "anhui20_705_b20_fl72"
base_project_name = [f"{base_project_prefix}_seed{seed}" for seed in seeds]
csv_path = ["./Data/Select/anhui20_705.csv"] * len(base_project_name)
...
def process_seed(seed):
    """处理单个种子的评估任务"""
    current_project_name = f"{base_project_prefix}_seed{seed}"
    print(f"开始处理种子: {seed} (项目: {current_project_name})")
    step1_extract_validation_metrics(current_project_name)
    step2_run_training_evaluation(f"./Data/Select/anhui20_705.csv", current_project_name)
    step3_merge_training_validation_data(current_project_name)
    step4_standardize_data(current_project_name)
    print(f"完成种子: {seed} 的处理")


def main():
    """主函数：批量执行多个seed的评估流水线"""
    print("开始批量执行LSTM模型性能评估流水线...")
    ...
    try:
        # 使用多进程并行处理种子
        with Pool(processes=3) as pool:  # 设置最大并行数为 4
